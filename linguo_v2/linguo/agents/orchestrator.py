...
On startup:
  1. VocabLoader seeds the RAG dictionary with all curated JSON vocab entries (guaranteed correct romanization).
  2. SQLiteStore loads vocab, stats, and RAG entries from disk → restores UserState.
...
from data.vocab.vocab_loader import VocabLoader
...
class Orchestrator:
    def __init__(self):
        ...
        # ── Curated vocabulary — loaded before RAG so seed_rag() can run ──────
        self.vocab = VocabLoader()

        # ── RAG + state: restored from SQLite, then seeded from vocab JSON ────
        self.rag     = RAGDictionary()
        n_seeded     = self.vocab.seed_rag(self.rag)
        self.state   = self._load_state_from_db()

        print(f"[orchestrator] vocab loaded: {n_seeded} entries seeded into RAG "
              f"across {len(self.vocab.languages())} languages")

        # ── Agents ────────────────────────────────────────────────────────────
        self.sentence_agent  = SentenceAgent(self.rag, self.memory, self.vocab)
